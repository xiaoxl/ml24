...
costJ = [f(np.array([[p[0]]]), np.array([[p[1]]]), x, y)[0,0]]
...
for _ in range(200):
    ...
    p = p + d
    costJ.append(f(np.array([[p[0]]]), np.array([[p[1]]]), x, y)[0,0])
...
```
The cost is close to $0$ after 200 iterations and seems to be convergent. Therefore we believe that we are close to the minimal point. The point we get is `{python} (bestp[0].item(), bestp[1].item())`.
